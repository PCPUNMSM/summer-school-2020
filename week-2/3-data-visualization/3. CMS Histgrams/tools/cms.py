...
def draw_jet(
    origin=(0, 0), angle=90, length=0.5, opening_angle=20, ntracks=5, show_tracks=False
):
    lines = []
    patches = []
...
def draw_jets(
    ...
    # Edges of cone
    for origin, angle, length, opening_angle, ntracks, show_tracks in zip(
        origins, angles, lengths, opening_angles, ntrackss, show_trackss
    ):
...
def draw_beams():
    ...
    lines = draw_line3D(
        origin=[(0, 0, -0.1), (0, 0, 0.1)],
        pmom=[(0, 0, -200.0), (0, 0, 200.0)],
        color="red",
        lw=1,
    )
